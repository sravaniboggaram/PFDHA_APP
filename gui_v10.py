...
from profile_evaluation_v4 import EvalCoordinator
from PyQt5.QtGui import QIcon, QFont
from dataclasses import dataclass, field
from shutil import rmtree
from ResizeImage import ScaledImageLabel
from profile_metric_widget import ProfileMetricGraphWidget
from profile_map_widget import ProfileMapWidget
...
class ProcessingRun:
    ...
    coords: pd.DataFrame = field(default_factory=lambda:
                                 pd.DataFrame(columns=["ID",
                                                       ...
                                                       "file_idx",
                                                       "profile_idx"]))
    coords_row_lookup: dict = field(default_factory=dict)

    map_software: str = "leaflet"
    map_dimension: str = "DISP1"
...
class MapBridge(QObject):
    ...
    @pyqtSlot(int, int)
    def show_profile(self, file_idx, profile_idx):
        run = self.main_window.active_run

        if run is None:
            return

        if not 0 <= file_idx < len(run.file_list):
            return

        profiles = run.file_list[file_idx]

        if not 0 <= profile_idx < len(profiles):
            return

        if profiles[profile_idx] is None:
            return

        self.main_window.display_profile(
            file_idx,
            profile_idx,
        )

        if hasattr(self.main_window, "results_tabs"):
            self.main_window.results_tabs.setCurrentWidget(
                self.main_window.profile_screen_split
            )
...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.map_view_action = QAction("Map View", self)
        self.map_view_action.triggered.connect(self.open_map_view)
        self.map_view_action.setEnabled(False)
        tools_menu.addAction(self.map_view_action)
    ...
    def show_losses_tab(self):
        if hasattr(self, "results_tabs") and hasattr(self, "loss_graph_widget"):
            self.results_tabs.setCurrentWidget(self.loss_graph_widget)
            return
        elif hasattr(self, "results_tabs") and not hasattr(self, "loss_graph_widget"):
            self.loss_graph_widget = ProfileMetricGraphWidget(self, metric="loss")
            self.results_tabs.addTab(self.loss_graph_widget, "Losses")
        else:
            self.show_results_tabs("LOSS")

        self.loss_graph_widget.refresh(rebuild_tree=True)
        self.results_tabs.setCurrentWidget(self.loss_graph_widget)


    def show_disp_tab(self):
        if hasattr(self, "results_tabs") and hasattr(self, "disp_graph_widget"):
            self.results_tabs.setCurrentWidget(self.disp_graph_widget)
            return
        elif hasattr(self, "results_tabs") and not hasattr(self, "disp_graph_widget"):
    ...
    def show_results_tabs(self, metric):
        """
        Create the main results tab workspace:

            Profiles | Losses | Displacement

        The Profiles tab reuses self.profile_screen_split.
        Losses and Displacement use ProfileMetricGraphWidget.
        """
        ...
        if metric == "LOSS":
            ...
            self.map_widget = ProfileMapWidget(self, MapBridge)
            self.results_tabs.addTab(self.map_widget, "Map")


        self.main_content_layout.addWidget(self.results_tabs)

    def initialize_run_coords(self, run):
        profile_keys = [
            (file_idx, profile_idx)
            for file_idx, profiles in enumerate(run.file_list)
            for profile_idx in range(len(profiles))
        ]

        run.coords = pd.DataFrame({
            "ID": pd.Series([pd.NA] * len(profile_keys), dtype="string"),
            "LAT": np.full(len(profile_keys), np.nan),
            "LONG": np.full(len(profile_keys), np.nan),
            "DISP1": np.full(len(profile_keys), np.nan),
            "DISP2": np.full(len(profile_keys), np.nan),
            "WIDTH": np.full(len(profile_keys), np.nan),
            "file_idx": [key[0] for key in profile_keys],
            "profile_idx": [key[1] for key in profile_keys],
        })

        run.coords_row_lookup = {
            key: row_idx for row_idx, key in enumerate(profile_keys)
        }
    ...
    def open_map_view(self):
        ...
        if (
            ...
            or run.coords is None
            or run.coords.empty
        ):
            return

        if hasattr(self, "results_tabs") and hasattr(self, "map_widget"):
            self.map_widget.refresh()
            self.results_tabs.setCurrentWidget(self.map_widget)
            return
        elif hasattr(self, "results_tabs") and not hasattr(self, "map_widget"):
            self.map_widget = ProfileMapWidget(self, MapBridge)
            self.results_tabs.addTab(self.map_widget, "Map")
        else:
            self.show_results_tabs("MAP")

        self.map_widget.refresh()

        self.results_tabs.setCurrentWidget(
            self.map_widget
        )

    def update_map_availability(self):
        run = self.active_run

        has_valid_coordinates = (
            isinstance(run.coords, pd.DataFrame)
            and {"LAT", "LONG"}.issubset(run.coords.columns)
            and run.coords[["LAT", "LONG"]].notna().all(axis=1).any()
        )

        available = (
            run is not None
            and run.status == "complete"
            and isinstance(run.coords, pd.DataFrame)
            and has_valid_coordinates
        )

        self.map_view_action.setEnabled(available)

        if (
            hasattr(self, "results_tabs")
            and hasattr(self, "map_widget")
        ):
            map_index = self.results_tabs.indexOf(self.map_widget)

            if map_index >= 0:
                self.results_tabs.setTabEnabled(map_index, available)

                if (
                    not available
                    and self.results_tabs.currentWidget()
                    is self.map_widget
                ):
                    self.results_tabs.setCurrentWidget(self.profile_screen_split)

    def add_profile_to_run_map(self, run, result):
        table = result.get("table")

        if (
            not isinstance(table, pd.DataFrame)
            or not {"LAT", "LONG"}.issubset(table.columns)
        ):
            return

        latitudes = pd.to_numeric(table["LAT"], errors="coerce").dropna()
        longitudes = pd.to_numeric(table["LONG"], errors="coerce").dropna()

        if latitudes.empty or longitudes.empty:
            return

        latitude = float(latitudes.iloc[0])
        longitude = float(longitudes.iloc[0])
        ...
        width_column = "actual width"
        widths = pd.to_numeric(table[width_column],errors="coerce").dropna()

        if not widths.empty:
            width = float(widths.iloc[0])

        disp_by_dimension = (
            table.groupby("Dimension", sort=True)["disp"].sum()
        )

        disp_values = pd.to_numeric(disp_by_dimension, errors="coerce").dropna().to_numpy(dtype=float)

        disp1 = disp_values[0]
        disp2 = disp_values[1] if len(disp_values) > 1 else None

            #if len(disp_values):
                #Use magnitude across dimensions to grade marker size.
                # displacement = float(
                #     np.linalg.norm(
                #         disp_values
                #     )
        ...
        profile_id = result.get("file_num")

        if profile_id is None:
            try:
                profile_id = run.loss_x_vals[file_idx][profile_idx]
            except Exception:
                profile_id = profile_idx + 1
        ...
        # run.coords = pd.concat([run.coords, pd.DataFrame([row])], ignore_index=True)   

        row_idx = run.coords_row_lookup[(file_idx, profile_idx)]

        run.coords.loc[
            row_idx, ["ID", "LAT", "LONG", "DISP1", "DISP2", "WIDTH"]
        ] = [str(profile_id), latitude, longitude, disp1, disp2, width]
    ...
    def create_proc_panel_widgets(self):
        ...
        cpu_placeholder_txt = f"{cpu_count()}"
        ...
        gpu_placeholder_txt = f"{torch.cuda.device_count()}"
        ...
        def cpu_gpu_placeholder_update():
            if radio_gpu.isChecked():
                cores_line_edit.setPlaceholderText(gpu_placeholder_txt)
            else:
                cores_line_edit.setPlaceholderText(cpu_placeholder_txt)

        cpu_gpu_placeholder_update()
        cpu_gpu_group.buttonClicked.connect(cpu_gpu_placeholder_update)
        ...
        # Data Resolution
        resolution_label = QLabel("Data Resolution")
    ...
    def process(self):
        ...
        self.initialize_run_coords(run)
    ...
    def process_locations(self, row, file_num):
        if row is None and self.loc_file is None:
            return None

        if self.loc_file is not None:
            if self.coord_point1 not in self.loc_file.columns:
                if self.loc_format == "UTM" and "lat" in self.loc_file.columns:
                    self.loc_format = "LATLON"
                    self.coord_point1, self.coord_point2 = "lat", "lon"
                elif self.loc_format == "LATLON" and "easting" in self.loc_file.columns:
                    self.loc_format = "UTM"
                    self.coord_point1, self.coord_point2 = "easting", "northing"
                else:
                    self.loc_format = None
                    return None
        ...
        coords = [self.loc_format,
                  row[self.coord_point1].astype(float),
                  row[self.coord_point2].astype(float),
                  row[self.az].astype(float)] # E,N,A or Lat, Lon, A
            
        if self.loc_format == 'UTM':
            coords.extend([int(row['ZN'].iloc[0]),
                           str(row['ZL'])])
    ...
    def read_txt(self, is_file, folder, num_files, folder_name, run):
        ...
        while i < num_files:
            ...
            if prof_id == "None":
                ...
                coords = self.process_locations(df.iloc[0], file_name) if self.loc_format else None
                ...
                profile_ids = list(profiles.groups.keys())
    ...
    def read_h5(self, folder, n_files, run):
        ...
        for curr_file_i in range(n_files):
            ...
            for k in keys:
                ...
                coords = self.process_locations(None, key) if self.loc_format else None
    ...
    def on_profile_result_ready_for_run(self, run, r):
        ...
        if self.loc_format is not None:
            self.add_profile_to_run_map(run, result=r)
