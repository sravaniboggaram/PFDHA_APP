...
import re
from helper_funs import calc_uncertainties
...
def weighted_loss(model, x, y_act):
    eps = 1e-6
    w_min = 0.5
    w_max = 3.0
    ...
    y = y_pred.view(n,-1).mean(dim=1)

    gradient = torch.autograd.grad(outputs=y.sum(), inputs=x, create_graph=True)[0]
    #gradient_2 = torch.autograd.grad(gradient.sum(), inputs=x, create_graph=True)[0].abs().detach()
    w = (1.0/(gradient.abs() + eps)).detach()
    ...
    return torch.mean(w*(y_act - y_pred)**2)


def mixed_weighted_loss(x, y_pred, y_act):
    eps = 1e-6
    w_min = 0.5
    w_max = 3.0
    ...
    y = y.view(n,-1).mean(dim=1)
    gradient = torch.autograd.grad(outputs=y.sum(), inputs=x, create_graph=True)[0].abs().detach()
    coeff = gradient / (1.0 + gradient)
    w = (1.0/(gradient.abs() + eps)).detach()
    w = torch.clamp(w, w_min, w_max)
    w = w / w.mean()
    ...
    return torch.mean(w*(coeff*r.abs() + (1-coeff)*(r**2)))


def regional_weighted_loss(y_pred, y_act, loc_params, width_params,j, scale):
...
def NN_optimize(data, collect_param_vals=False):
    ...
    x_tensor = torch.tensor(x, dtype=float, requires_grad=True).unsqueeze(0).T
    ...
    loss_fn = regional_weighted_loss

    scheduler1 = torch.optim.lr_scheduler.LinearLR(opt)
    scheduler2 = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, 'min')

    y_act = torch.tensor(y, dtype=float)
    y_pred = model(x_tensor)
    
    prev_loss = loss_fn(y_pred, y_act, loc_p, width_p,0, scale).item()
    ...
    while True:
        ...
        if iter_n > 1000:
            loss_fn = regional_weighted_loss
        loss = loss_fn(y_pred, y_act, loc_p, width_p, iter_n, scale)
        opt.zero_grad()
        loss.backward()
        opt.step()
