...
from PyQt5.QtGui import QColor, QPainter, QBrush, QIcon, QFont, QPixmap
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.disp_graph_vals = []
        self.loss_graph_vals = []
    ...
    def get_list_mode(self, vals_list):
        """
        Returns:
        ...
        # if not getattr(self, "loss_list", None):
        #     return "empty"
    ...
    def build_losses_tab(self):
        ...
        mode = self.get_list_mode(self.loss_graph_vals)
        ...
        if mode == "folder_single_profiles" or mode == "single_file_multi_profiles":
            fig = self.make_folder_or_single_nprof_fig(self.loss_graph_vals, mode)
    ...
    def make_folder_or_single_nprof_fig(self, vals_list, mode):
        fig = Figure(figsize=(8, 5))
        ax = fig.add_subplot(111)

        if mode == "single_file_multi_profiles":
            vals_list = self.loss_list[0]
        
        x = list(range(1, len(vals_list) + 1))

        ax.plot(x, vals_list, marker="o")
    ...
    def init_loss_file_selection_state(self):
        if hasattr(self, "loss_file_order") and hasattr(self, "loss_file_checked"):
            return

        self.loss_file_order = list(range(len(self.loss_list)))
        self.loss_file_checked = {
            file_idx: True
            for file_idx in self.loss_file_order
        }
    ...
    def build_multi_file_loss_selector(self):
        ...
        self.loss_file_list_widget.setDragDropMode(QAbstractItemView.InternalMove)

        self.populate_loss_file_list_widget()

        update_btn = QPushButton("Update Loss Plot")
        update_btn.clicked.connect(self.update_multi_file_loss_plot_from_list)

        left_panel = QWidget()
        left_layout = QVBoxLayout(left_panel)
        left_layout.addWidget(QLabel("Select files to include"))
    ...
    def populate_loss_file_list_widget(self):
        ...
        for file_idx in self.loss_file_order:
            ...
            checkbox.setChecked(self.loss_file_checked.get(file_idx, True))
            ...
            self.loss_file_list_widget.setItemWidget(item, checkbox)

    def set_loss_file_checked(self, file_idx, state):
        self.loss_file_checked[file_idx] = state == Qt.Checked

    def get_current_loss_file_order_from_widget(self):
        order = []

        for row in range(self.loss_file_list_widget.count()):
            item = self.loss_file_list_widget.item(row)
            file_idx = item.data(Qt.UserRole)
            order.append(file_idx)

        return order
    

    def update_multi_file_loss_plot_from_list(self):
        self.loss_file_order = self.get_current_loss_file_order_from_widget()

        fig = self.make_multi_file_loss_fig(
            selected_file_indices=[
                file_idx
                for file_idx in self.loss_file_order
                if self.loss_file_checked.get(file_idx, True)
            ]
        )
    ...
    def make_multi_file_loss_fig(self, selected_file_indices):
        ...
        for file_idx in selected_file_indices:
            file_losses = self.loss_list[file_idx]

            if not isinstance(file_losses, list):
                continue

            clean_losses = [
                loss for loss in file_losses
                if loss is not None
            ]
            ...
            if clean_losses:
                labels.append((start_x + end_x) / 2)

        ax.plot(x_vals, y_vals, marker="o")

        ax.set_title("Final Loss by Selected Files")
        ax.set_xlabel("Profiles in selected file order")
        ax.set_ylabel("Final Loss")
        ax.grid(True)
    ...
    def process(self):
        ...
        if self.curr_path.suffix == '.h5':
            jobs = self.read_h5(folder, num_files)
            cols_data = None
        else:
            jobs, cols_data = self.read_txt(is_file, folder, num_files, folder_name)
        ...
        if self.temp_fig_folder.is_dir():
            rmtree(self.temp_fig_folder)
        self.temp_fig_folder.mkdir(parents=True)
        ...
        self.config = EvalConfig(
            ...
            file_format=self.curr_path.suffix,
            txt_cols_data=cols_data,
            temp_fig_folder=self.temp_fig_folder,
            interp=interp
        )
    ...
    def build_profile_nav_cache(self):
        ...
        self.profile_nav_positions = []
        self.profile_nav_lookup = {}

        for file_idx, file_item in enumerate(self.file_list):
            if isinstance(file_item, list):
                for profile_idx in range(len(file_item)):
                    flat_idx = len(self.profile_nav_positions)

                    self.profile_nav_positions.append(None)
                    self.profile_nav_lookup[(file_idx, profile_idx)] = flat_idx

            else:
                flat_idx = len(self.profile_nav_positions)

                self.profile_nav_positions.append(None)
                self.profile_nav_lookup[(file_idx, None)] = flat_idx

    def get_h5_subset(self, num_files):
        dialog = select_h5_subset.SelectH5Subset(num_files)
        if dialog.exec_() == QDialog.Accepted:
            subset_text = dialog.selected_options()
            if subset_text is None:
                return "ALL"
            for i in range(len(subset_text)):
    ...
    def read_txt(self, is_file, folder, num_files, folder_name):
        ...
        self.disp_graph_vals = single_prof_count*[None]
        self.loss_graph_vals = single_prof_count*[None]
        t = 0
        for count in n_profs_counts:
            t += count
            self.file_list.append(count*[None])
            self.disp_graph_vals.append(count*[None])
            self.loss_graph_vals.append(count*[None])
    ...
    def read_h5(self, folder, num_files):
        ...
        for curr_file_i in range(num_files):
            ...
            self.disp_graph_vals = [[] for _ in range(len(groups))]
            ...
            self.disp_graph_vals.append(len(keys)*[None])
            self.loss_graph_vals.append(len(keys)*[None])
            profile_list_idx = 0
            for k in keys:
                key = prof_keys[k]
    ...
    def start_evaluation(self, jobs, device, n_cores):
        ...
        self.worker.result_ready.connect(self.on_profile_result_ready)
    ...
    def on_profile_result_ready(self, r):
        ...
        if file_idx is None:
            self.file_list[prof_idx] = r
            nav_key = (prof_idx, None)
            self.loss_graph_vals[prof_idx] = r['final_loss']
        else:
            self.file_list[file_idx][prof_idx] = r
            nav_key = (file_idx, prof_idx)
            self.loss_graph_vals[file_idx][prof_idx] = r['final_loss']

        if hasattr(self, "losses_tab") and self.losses_tab is not None:
            self.refresh_losses_tab_after_result()

        flat_idx = self.profile_nav_lookup.get(nav_key)
        if flat_idx is not None:
            self.profile_nav_positions[flat_idx] = nav_key

        # First result: build the selector UI
        if not getattr(self, "selector_view_initialized", False):
    ...
    def completed_profile_positions(self):
        """
        Yield every completed profile position in self.file_list order.
        ...
        for file_idx, file_item in enumerate(self.file_list):
            ...
            # Multi-profile file
            if isinstance(file_item, list):
                for profile_idx, profile in enumerate(file_item):
                    if profile is not None:
                        yield file_idx, profile_idx
    ...
    def get_adjacent_profile_position(self, file_index, profile_index, direction):
        ...
        if not hasattr(self, "profile_nav_lookup"):
            return None

        current_flat_idx = self.profile_nav_lookup.get(current_key)
        ...
        while 0 <= i < len(self.profile_nav_positions):
            pos = self.profile_nav_positions[i]
    ...
    def build_profile_view(self, file_index, profile_index, flag=False):
        if profile_index is None:
            profile = self.file_list[file_index]
        else:
            profile = self.file_list[file_index][profile_index]
