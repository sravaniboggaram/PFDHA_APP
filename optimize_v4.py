...
def _dimension_rupture_param_count(n_rup):
    """Effective parameter count for asking whether one dimension alone justifies rupture structure."""
    return 2 + 4 * int(n_rup)  # origin/ramp + loc/width/disp/slope per rupture


def _dimension_signal_test(x, y_true, joint_pred, n_rup):
    """Compare each dimension's joint rupture prediction with a 2-parameter linear/no-rupture baseline using BIC."""
    tests = []
    ...
    for dim in range(y_true.shape[1]):
        ...
        tests.append({
            "dimension": dim, "has_signal": bool(rupture_bic < linear_bic), "linear_bic": linear_bic,
            "rupture_bic": rupture_bic, "delta_bic_rupture_minus_linear": rupture_bic - linear_bic,
...
def _shared_gradient_conflict(model, x, y_true, device):
    ...
    pred = model(x_tensor)
    losses = [torch.mean((pred[:, d] - y_tensor[:, d]) ** 2) for d in range(2)]
    grad_vectors = []

    try:
        for d in range(2):
            grads = torch.autograd.grad(losses[d], shared, retain_graph=d == 0, allow_unused=True)
...
def _replace_dimension_table(table, dim_table, dim):
    if "Dimension" not in table.columns or "Dimension" not in dim_table.columns:
        return table
    dim_table = dim_table.copy()
    dim_table["Dimension"] = dim
    out = pd.concat((table.loc[table["Dimension"] != dim].copy(), dim_table), ignore_index=True)
...
def _remove_no_signal_dimensions(table, no_signal_dims):
    """Remove no-signal fit rows. If every dimension is no-signal, keep one metadata row with fit fields set to NaN."""
    if not no_signal_dims or "Dimension" not in table.columns:
        return table

    out = table.loc[~table["Dimension"].isin(no_signal_dims)].copy()
    if not out.empty:
    ...
    fit_names = {
        "rupture", "disp", "displacement", "width", "actual width", "loc", "rupture loc",
...
def run_optimization(smooth_data, orig_data, prof_num, sigma, rand=False, uncert=False, coords=None, win_bounds=None,
    ...
    Staged model selection:
      ...
      5) expensive independent refits require both a residual problem and opposing shared loc/width gradients.
    """
    ...
    if data_obj.n_dim == 2:
        signal_tests = _dimension_signal_test(x_eval, y_eval, pred_eval, data_obj.n_rup)
    ...
    if selected_independent_dims:
        ...
        for dim in range(data_obj.n_dim):
            if dim in selected_independent_dims:
                ...
                table = _replace_dimension_table(table, dim_table, dim)
    ...
    table = _remove_no_signal_dimensions(table, no_signal_dims)
